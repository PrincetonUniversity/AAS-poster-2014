...
\title{Investigating mass transfer in symbiotic systems with
...
realistic 3D hydrodynamical simulation of the interaction of the wind from a
giant star with a companion using the spherical coordinates version of the code
is under progress. The computational domain spans about one radian around the
orbital plane of the system centered on the primary to cover the whole region
of interest close to the accretor. The resolution per separation in the 3D
simulations is about a factor of two smaller than in the 2D simulations and
leads to the relative damping of the bow shock and spiral structures.
}
...
\begin{itemize}
  \item The focused wind accretion in this system show a complex dynamics
    that proceeds beyond the accreting companion.
  ...
  \item Mass transfer through a focused wind leads to a stream flow onto the WD
    with variable accretion rates of a $\sim$ 5\% percent of the mass loss
    from the primary.
...
solution by modifying the spacing of the grid points in certain regions while
the system is evolved.  The AMR algorithm places high resolution grid patches
...
at the STScI, which is operated by the AURA, Inc.).  We acknowledge partial
support from grants NSF AST-1108686 and NASA NNX12AH91H.  The numerical code
used in this work is based on the \flash{} code that is in part developed by
the DOE NNSA-ASC OASCR Flash Center at the University of Chicago.
}
